...
class FullTextOverviewPage extends StatefulWidget {
  const FullTextOverviewPage({super.key});

  @override
  State<FullTextOverviewPage> createState() => _FullTextOverviewPageState();
}

class _FullTextOverviewPageState extends State<FullTextOverviewPage> {
  String? _selectedFolder;
  String? _globalSummaryContent;
  ...
  void _onNovelSelected(String? folderName) {
    // 如果有未保存的更改，提示用户
    if (_selectedFolder != null && _globalSummaryContent != _textController.text) {
      _confirmSaveBeforeChange(() {
        setState(() {
          _selectedFolder = folderName;
          _globalSummaryContent = null;
          _contentExists = false;
          _textController.text = '';
        });

        if (folderName != null) {
          _loadGlobalSummaryContent(folderName);
        }
      });
    } else {
      setState(() {
  ...
  // 加载全文概述内容
  Future<void> _loadGlobalSummaryContent(String folderName) async {
    setState(() {
      _isLoading = true;
    });

    try {
      final content = await NovelFileService().readGlobalSummary(folderName);
      setState(() {
        _globalSummaryContent = content;
  ...
  void _scheduleSave() {
    ...
    _saveTimer = Timer(const Duration(seconds: 2), () {
      if (_selectedFolder != null && _globalSummaryContent != _textController.text) {
        _saveGlobalSummaryContent(_selectedFolder!, _textController.text);
      }
    });
  }

  // 保存全文概述内容
  Future<void> _saveGlobalSummaryContent(String folderName, String content) async {
    setState(() {
      _isSaving = true;
    });
    
    try {
      final success = await NovelFileService().saveGlobalSummary(folderName, content);
      if (success) {
        setState(() {
          _globalSummaryContent = content;
          _contentExists = true;
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                if (_selectedFolder != null) {
                  await _saveGlobalSummaryContent(_selectedFolder!, _textController.text);
                  onConfirmed();
                }
              },
            ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_selectedFolder == null)
            ...
            else if (!_contentExists && _globalSummaryContent == null)
              // 文件不存在时显示提示
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                ...
                child: Text(
                  localizations.translate('full_text_overview_not_exists'),
                  style: const TextStyle(fontFamily: 'Microsoft YaHei'),
                ),
              )
            else
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: localizations.translate('full_text_overview_hint'),
